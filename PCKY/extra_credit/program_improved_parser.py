...
class PCKY:
    ...
    def parse_sentence(self,sentence):
        ...
        #going from left to right
            ...
            '''
            #If terminal is not found in vocabulary, find the corresponding tag/LHS using nltk.pos_tag
            if len(trees) == 0:
                trees.append(nltk.ProbabilisticTree(prod.lhs().symbol(), [tuple(['UNK',])], prob=prod.logprob()))
                print("Terminal not found, using NLTK POS to identify the tag")
                text = nltk.word_tokenize(tokens[j-1])
                y = nltk.pos_tag(text)[0]
                trees.append(nltk.ProbabilisticTree(nltk.grammar.Nonterminal(str(y[1])), [tokens[j-1]], prob=0))                
            '''
...
if __name__ == '__main__':
    if (len(sys.argv) > 2):
        ...
        input_PCFG_file = '../data/pfcg_unk.out'
        test_sentence_filename = '../data/sentences.txt'
        output_parse_filename = '../data/parse_sentences_oov.out'
    ...
    parser = PCKY(grammar) #initialize the PCKY class
    ...
    with open(output_parse_filename,'w') as write_file:
        for sentence in sentences:
            ...
            if len(parses) == 0:
                ...
                final_tree = parses[0]
                s = final_tree.pformat( parens='()', quotes=False)
                s = s.split('\n')
                s = [item.strip() for item in s]
                flat_tree = ' '.join(s)

                s1 = Tree.fromstring(flat_tree)
                s1.un_chomsky_normal_form(expandUnary=True, parentChar="^") #use un_chomsky_normal_form to get original tree format from the parent-annotated tree

                s = s1.pformat( parens='()', quotes=False)
                s = s.split('\n')
                s = [item.strip() for item in s]
                flat_tree = ' '.join(s)
